...
from ml_only.loaddata_old import load_cesm2_by_period
...
# Reverse weight
wgt = np.sqrt(np.cos(np.deg2rad(y.lat)))
wgt_mean = wgt.where(y.isel(sample=0).notnull()).mean()
y = y * wgt_mean / wgt
